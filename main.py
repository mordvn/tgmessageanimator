...
async def ad_scroll_animation(message: types.Message, ad_text: str, duration_str: str, width_str=None):
...
async def timer_animation(message: types.Message, duration_str: str, timer_text=None):
    ...
    while datetime.now() < end_time:
        ...
        timer_display = (
            f"{timer_text}\n"
            f"{elapsed_str} {bar} {remaining_str}"
        )
...
@router.business_message(lambda message: message.text and message.text.startswith('.type '))
...
@router.business_message(lambda message: message.text and message.text.startswith('.timer '))
async def handle_timer_command(message: types.Message):
    ...
    if not duration:
        await message.answer("Please provide a duration. Example: .timer 30m or .timer \"Meeting\" 30m")
        return
    
    asyncio.create_task(timer_animation(message, duration, timer_text))

@router.business_message(lambda message: message.text and message.text.startswith('.fbi'))
...
@router.business_message(lambda message: message.text and message.text.startswith('.ad '))
async def handle_ad_command(message: types.Message):
    parts = message.text[4:].strip().split()
    
    if len(parts) < 2:
        await message.answer("Please provide ad text and duration. Example: .ad \"Selling something\" 10m 20ch")
        return
